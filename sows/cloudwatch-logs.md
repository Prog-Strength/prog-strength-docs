...
2. **`AWS_REGION` is required in the host's `.env`.** The compose files use `${AWS_REGION:?...}`, which fails fast at `docker compose up` if the var is unset with a clear error message. This was originally a soft default (`${AWS_REGION:-us-east-1}`) that silently shipped logs cross-region on the first deploy — the strict form is the lesson learned. Add `AWS_REGION=us-east-2` (or whatever region your Terraform points at) to the deploy workflow's secret-derived `.env`. The driver region MUST match the region the log groups were created in, otherwise CreateLogStream returns 403 against a group that doesn't exist in the region the driver is targeting.
