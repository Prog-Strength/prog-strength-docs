...
- `(user_id, position ASC)` — the dominant read query: "give me this user's selection in display order."
- A unique index on `(user_id, position)` to prevent two rows from sharing the same slot within a user. The write path always rewrites positions densely from 0 inside the same transaction that deletes the prior rows, so this invariant is easy to maintain.
...
1. **Drag-and-drop reordering vs. checkbox-only in the modal.** The schema supports ordering today via the `position` column, and the API accepts an ordered array, so the backend is reorder-ready regardless. The question is whether v1 of the modal ships with drag-and-drop or just toggles (preserving the order in which the user checked things). **Lean**: checkboxes only for v1 — drag-and-drop is finicky on mobile and the win is small for a list that caps at 12. Add reordering as a follow-up if users ask.

2. **Maximum selection cap.** Twelve is the working number above. Smaller (e.g. eight) keeps the Personal Records grid clean on phones; larger (e.g. twenty) gives competitive lifters more headroom. **Lean**: 12. Round number, fits comfortably in a 3-column grid on tablet and a 2-column grid on phone, and matches the "what a serious lifter actually tracks weekly" intuition. Easy to bump if it shows up as a complaint.

3. **`is_default` flag in the read response.** Useful for the modal — it can render a small "default" badge so users know which lifts are part of the curated set when they're picking. Costs one cheap set membership check per row at the handler. **Lean**: include it. Cheap, removes the need for the frontend to fetch the defaults list a second time when rendering the modal.

4. **Behavior when a user PUTs the default list verbatim.** Two semantics are defensible: (a) store the rows just like any other customization — the user has explicitly chosen this set, which happens to match defaults today; or (b) detect that the list matches defaults and delete all rows instead, so the user falls back to whatever future change ships in `HeadlineExercises`. **Lean**: (a). It is simpler, more predictable for the user ("I saved these lifts and they're saved"), and matches the way set-replacement endpoints conventionally work elsewhere in the API. The cost is that a user who wants to track the defaults forever-as-defaults has to re-PUT after the curated set changes; in practice the curated set will rarely change, and a follow-up "follow defaults" toggle is a separate feature when it matters.

5. **Catalog removal of a slug a user has selected.** Today the catalog is admin-curated and effectively append-only, so this is a theoretical concern. If it ever happens, the FK has no `ON DELETE` clause and the catalog removal would fail until the dependent rows are cleaned up — which is the conservative default. **Lean**: leave as-is; revisit if the catalog ever grows a real removal path.
